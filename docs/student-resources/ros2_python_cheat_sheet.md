...
This cheat sheet provides quick reference for common ROS 2 commands and Python code snippets.

## ROS 2 CLI Commands

### Workspace Management
-   `colcon build`: Build packages in current workspace.
-   `colcon build --packages-select <pkg_name>`: Build specific package.
-   `source install/setup.bash`: Source workspace making packages available.
...
-   `ros2 node info <node_name>`: Get info about specific node.
-   `ros2 topic list`: List active ROS 2 topics.
-   `ros2 topic info <topic_name>`: Get info about specific topic.
-   `ros2 topic echo <topic_name>`: Display messages on topic.
-   `ros2 service list`: List active ROS 2 services.
-   `ros2 service type <service_name>`: Get type of service.
-   `ros2 action list`: List active ROS 2 actions.

### Communication Tools
-   `ros2 topic pub <topic> <msg_type> <args>`: Publish data to topic.
-   `ros2 service call <service> <srv_type> <args>`: Call service.
...
*(Additional content will be incorporated as course evolves.)*
